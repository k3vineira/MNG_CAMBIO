    ...
    function cargarDepartamentos(selectedCountryCode) {
        ...
        if (selectedCountryCode === 'COL' || selectedCountryCode === 'Colombia') {
                ...
                fetchJSON('https://www.datos.gov.co/resource/gdxc-w37w.json?$limit=1500')
                    .then(function (data) {
                        ...
                        data.forEach(function (item) {
                            ...
                            var deptCode = item.c_digo_dane_del_departamento;
                            var muniName = formatLocationName(item.nom_mpio);
                            var muniCode = item.c_digo_dane_del_municipio;
            ...
            .then(function (data) {
                ...
                if (states.length === 0) {
                    deptoSelect.innerHTML = '<option value="N/A" selected>No aplica</option>';
                    if (!readOnly) deptoSelect.disabled = true;
                    cargarCiudadesSinEstado(countryName);
                } else {
                    ...
                    states.forEach(function (state) {
                        var option = document.createElement('option');
                        option.value = state.state_code || state.name;
                    ...
                    if (deptoSelect.value) {
                        cargarCiudades(selectedCountryCode, deptoSelect.value);
                    } else if (!readOnly) {
            ...
            .catch(function (error) { console.error('Error estados:', error); cargarCiudadesSinEstado(countryName); });
        }
    }
